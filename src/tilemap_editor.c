...
// power pellet data
SDL_Point invalid_point = { -1, -1 };
idx2D invalid_idx = {-1, -1};
...
void mixAutoTilesAndOverrides(TileMap *tilemap, idx2D autoTileIdxs[TILE_ROWS][TILE_COLS], idx2D overrideIdxs[TILE_ROWS][TILE_COLS]) {
    for( int r = 0; r < TILE_ROWS; r++ ) {
        for( int c = 0; c < TILE_COLS; c++) {
            ...
            if( !idx2D_equal(overrideIdxs[r][c], invalid_idx ) ) {
...
int main( int argc, char *argv[] ) {
    ...
    while (!quit) {
        ...
        if ( left_button_pressed ) {
            ...
            if( current_mode == TILE_MODE ) {
                ...
                if( !tile_already_exists ) {
                    for( int i = 0; i < 4; ++i ) {
                        if( points_equal( levelConfig.powerPelletTiles[ i ], invalid_point ) ) {
                ...
                if( !tile_already_exists ) {
                    for( int i = 0; i < MAX_SLOW_TILES; ++i ) {
                        if( points_equal( tilemap.tm_slow_tiles[ i ], invalid_point ) ) {
        ...
        for( int i = 0; i < 4; ++i ) {
            if( points_equal( levelConfig.powerPelletTiles[ i ], invalid_point ) ) continue;
        ...
        for( int i = 0; i < MAX_SLOW_TILES; i++ ) {
            if( points_equal( tilemap.tm_slow_tiles[ i ], invalid_point ) ) continue;
        ...
        for( int r = 0; r < TILE_ROWS; r++ ) {
            for(int c = 0; c < TILE_COLS; c++) {
                if( !idx2D_equal(overrideTilemapTextureIdxs[r][c], invalid_idx) ) {
                    SDL_Rect rect = {
